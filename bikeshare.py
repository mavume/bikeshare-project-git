...
def get_filters():
    ...
    while True:
        ...
        if filter_by.lower().strip() == 'month':
            ...
            day = None
            break
        # get user input for day of week (all, monday, tuesday, Wednesday, Thursday, Friday, Saturday or sunday)
...
def time_stats(df):
    ...
    # display the most common day of week used
    try:
    ...
    print("\nRunning took %s seconds.\n" % (time.time() - start_time))
    print('-'*40)


def station_stats(df):
    """Shows statistics on the most popular stations and trip."""
    ...
    print("\nRunning took %s seconds." % (time.time() - start_time))
    print('-'*40)


def trip_duration_stats(df):
    """Shows statistics on the total and average trip duration."""
...
def raw_data (df):
    ...
    6 rows will added in each press"""
    print('Press enter to see row data, type no to skip or to stop')
    x = 0
    while (input()!= 'no'):
        x = x+6
        print(df.head(x))
